...
- Added `social-media-report` skill (private KGM skill, branded in the `kgm-kv` style) — turns Instagram, Facebook, and YouTube data (plus a manual spreadsheet for TikTok/LinkedIn/Google Meu Negócio, Phase 1) into a client-ready monthly performance report. Five-stage workflow: briefing → data collection (automated CLIs + manual spreadsheet via `xlsx`) → MoM calculation + automated insights (Awareness/Engagement/Conversion threshold framework extending the `social` skill's metrics framework) → deliverable assembly (Dashboard Artifact with a fixed redeployed link + condensed PDF, no PPTX) → QA/delivery checklist. Output follows the `KGM_Relatorio_NomeCliente_AAAAMM` naming convention with a "KGM Design e Comunicação" footer. References: `metrics-framework.md` (MoM formulas, per-platform thresholds, cross-metric signals, top/bottom post selection), `report-template.md` (11-section structure shared by dashboard and PDF), `manual-data-template.md` (Phase 1 spreadsheet format).
- Added `tools/clis/meta-insights.js` — zero-dependency CLI for the Instagram Graph API and Facebook Page Insights (organic data, the counterpart to `meta-ads.js`). Commands: `account get`/`account insights`, `media list`/`media insights`, `page get`/`page insights`/`page posts` (auto-exchanges a Page Access Token from the System User token), and a composite `report monthly` bundling Instagram account + insights + posts plus a `facebook_page` section, defaulting to the previous complete calendar month. Verified against the real Instagram Business and Facebook Page APIs.
- Added `tools/clis/youtube-insights.js` — zero-dependency CLI for the YouTube Data API v3 (public channel/video stats via an API key, no OAuth). Commands: `channel get`/`channel resolve` (handle → channel ID), `videos list`/`videos insights`, and a composite `report monthly`. Written to spec; pending verification against a real API key (see the integration guide's checklist).
- Added `tools/integrations/meta-insights.md` and `tools/integrations/youtube-insights.md` integration guides, and registered both tools in `tools/REGISTRY.md` and `tools/clis/README.md`.
- Total skills: 44.
